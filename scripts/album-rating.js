(function setupAlbumRating() {
  ...
  let user = null;
  let currentRating = 0;
  let _deletedRatings = {}; // Track rating deletions to prevent XP farming: { "user_id:album_id": timestamp }

  // Load deletion tracking from localStorage
  function loadDeletedRatingsCache() {
    try {
      const cached = localStorage.getItem('emdb_deleted_ratings_xp_track') || '{}';
      ...
      localStorage.setItem('emdb_deleted_ratings_xp_track', JSON.stringify(_deletedRatings));
    } catch (err) {
      console.log('[AR-RATING] deletion cache load failed:', err);
    }
  }
  ...
  function saveDeletedRatingsCache() {
    try {
      ...
      console.log('[AR-RATING] deletion cache save failed:', err);
    }
  }
  ...
  const applyUserRating = (value) => {
    ...
    setUserText(num > 0 ? `<span class="score-number">${num}</span><span class="score-suffix">/10</span>` : 'Rate this album');
  ...
  const saveRating = async (value) => {
    ...
    if (isFirstRating && window.EMDbXP && typeof window.EMDbXP.awardXP === 'function') {
      ...
      if (!deletionTime || (now - deletionTime) > recentDeletionWindow) {
        await window.EMDbXP.awardXP(user.id, 10, {
          action: 'album_rating_create',
          albumId
        }, supabaseClient);
      }
    }
